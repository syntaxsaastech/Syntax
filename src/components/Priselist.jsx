// src/components/Pricelist.jsx - Public Price List Page with PIN Protection
import React, { useState, useEffect } from 'react';
import { db, collection, query, orderBy, onSnapshot, getDocs } from '../firebase/config';

// Default PIN used if none is set in Firestore
...
function Pricelist() {
        ...
        const loadPin = async () => {
            try {
                ...
                if (!snapshot.empty) {
                    ...
                    setStoredPin(docData.data().pin || DEFAULT_PIN);
                } else {
    ...
    const handlePinSubmit = (e) => {
        ...
        if (enteredPin.length !== 4) {
            setPinError('⚠️ Please enter a 4-digit PIN.');
            return;
        }

        setIsVerifying(true);
        // Small delay so spinner is visible for better UX
        setTimeout(() => {
    ...
    useEffect(() => {
        if (enteredPin.length === 4 && !isVerifying && !isUnlocked) {
    ...
    }, [enteredPin, storedPin, isVerifying, isUnlocked]);
    ...
    if (!isUnlocked) {
        return (
            <>
                <style>{`
/* =========================================================
   PRICELIST PIN GATE — Multi-Color Premium Design
   ========================================================= */
...
.pl-gate-page::before {
    ...
    background:
        ...
        radial-gradient(circle at 50% 98%, rgba(77, 150, 255, 0.12) 0%, transparent 45%),
        radial-gradient(circle at 15% 75%, rgba(107, 203, 119, 0.10) 0%, transparent 42%),
        radial-gradient(circle at 85% 80%, rgba(255, 217, 61, 0.10) 0%, transparent 42%);
    pointer-events: none;
    z-index: 0;
...
.pl-gate-card {
    ...
    overflow: hidden;
    text-align: center;
    animation: plGateIn 0.6s cubic-bezier(0.16, 1, 0.3, 1);
}
...
.pl-gate-error {
    ...
    margin-bottom: 18px;
    text-align: center;
    animation: plShake 0.4s cubic-bezier(0.36, 0.07, 0.19, 0.97);
}
...
.pl-gate-pin-input::placeholder {
    color: #CBD5E1;
    letter-spacing: 0.6em;
}

.pl-gate-pin-input:focus {
    border-color: #6366F1;
    background: #FFFFFF;
    box-shadow: 0 0 0 5px rgba(99, 102, 241, 0.12);
}

.pl-gate-pin-input:disabled {
    opacity: 0.6;
    cursor: not-allowed;
}
...
.pl-gate-submit:hover:not(:disabled) {
    transform: translateY(-3px);
    background-position: 100% 50%;
    box-shadow: 0 16px 40px rgba(139, 92, 246, 0.45);
}

.pl-gate-submit:active:not(:disabled) {
    transform: translateY(-1px);
}

.pl-gate-submit:disabled {
    opacity: 0.55;
    cursor: not-allowed;
}
...
@media (max-width: 480px) {
    .pl-gate-page { padding: 100px 16px 40px; }
    .pl-gate-card { padding: 36px 24px; border-radius: 20px; }
    .pl-gate-icon { font-size: 3rem; }
    .pl-gate-title { font-size: 1.5rem; }
    .pl-gate-pin-input { font-size: 1.5rem; padding: 16px; }
}
`}</style>

                <div className="pl-gate-page">
                    <div className="pl-gate-inner">
                        <div className="pl-gate-card">
                            <span className="pl-gate-icon">🔒</span>
                            <h1 className="pl-gate-title">Protected Access</h1>
                            <p className="pl-gate-subtitle">
                                Enter the 4-digit PIN to view our pricing details
                            </p>

                            {pinError && <div className="pl-gate-error">{pinError}</div>}

                            <form className="pl-gate-form" onSubmit={handlePinSubmit}>
                                <div className="pl-gate-pin-wrap">
                                    <input
                                        className="pl-gate-pin-input"
                                        type="password"
                                        inputMode="numeric"
                                        autoComplete="off"
                                        maxLength="4"
                                        ...
                                        placeholder="••••"
                                        disabled={isPinLoading || isVerifying}
                                        autoFocus
                                    />
                                </div>

                                <div className="pl-gate-dots">
                                    {[0, 1, 2, 3].map((i) => (
                                        <span
                                            key={i}
                                            className={`pl-gate-dot ${enteredPin.length > i ? (pinError ? 'error' : 'filled') : ''}`}
                                        />
                                    ))}
                                </div>
                                ...
                                <button
                                    type="submit"
                                    className="pl-gate-submit"
                                    disabled={isPinLoading || isVerifying || enteredPin.length !== 4}
                                >
                                    {isVerifying ? (
                                        <>
                                            <span className="pl-gate-spinner" />
                                            Verifying...
                                        </>
                                    ) : isPinLoading ? (
                                        '⏳ Loading...'
                            ...
                            <div className="pl-gate-hint">
                                <span className="pl-gate-hint-icon">💡</span>
                                <span>Contact us if you don't have the PIN</span>
                            </div>
                        </div>
                    </div>
                </div>
