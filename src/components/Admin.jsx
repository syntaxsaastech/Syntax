// src/pages/Admin.js - COMPLETE FILE with History + Prices + Password Management
...
function Admin() {
    ...
    useEffect(() => {
        ...
        // Inject global reset + body background once
    ...
    const handleChangePassword = async (e) => {
        ...
        try {
            if (passwordDocId) {
                const docRef = doc(db, 'adminSettings', passwordDocId);
            ...
            setPasswordChangeError('❌ Error updating password: ' + error.message);
        }
    ...
    // Shared styles (NO @import - fonts loaded via useEffect link tag)
    const sharedStyles = `
        ...
        .admin-btn--password { color: #8C7AFF; border-color: rgba(140, 122, 255, 0.35); background: rgba(140, 122, 255, 0.08); }
        .admin-btn--password:hover { background: rgba(140, 122, 255, 0.16); border-color: #8C7AFF; }
        ...
        .admin-modal.p-password::before { background: linear-gradient(90deg, #4D96FF, #8C7AFF); animation: none; background-size: 100%; }
        ...
        .admin-modal.p-password .admin-modal-title { color: #4D96FF; }
            ...
            <div className="admin-page">
                ...
                <div className="admin-header">
                    ...
                    <div className="admin-header-actions">
                        ...
                        <button className="admin-btn admin-btn--password" onClick={() => setShowChangePasswordModal(true)}>🔐 Change Password</button>
                ...
                {showChangePasswordModal && (
                    <div className="admin-modal-overlay" onClick={() => { setShowChangePasswordModal(false); setPasswordChangeError(''); setPasswordChangeSuccess(''); setNewPassword(''); setConfirmPassword(''); }}>
                        <div className="admin-modal p-password" onClick={(e) => e.stopPropagation()}>
                            <div className="admin-modal-header">
                                <h2 className="admin-modal-title">🔐 Change Admin Password</h2>
                                <button className="admin-modal-close" onClick={() => { setShowChangePasswordModal(false); setPasswordChangeError(''); setPasswordChangeSuccess(''); setNewPassword(''); setConfirmPassword(''); }}>✕</button>
                            </div>

                            <div className="admin-note">
                                ⚠️ Note: This updates the admin password in Firestore. You must ALSO update the password in Firebase Auth console for full sync.
                            </div>

                            {passwordChangeError && <div className="admin-error">{passwordChangeError}</div>}
                            {passwordChangeSuccess && <div className="admin-success">{passwordChangeSuccess}</div>}

                            <form onSubmit={handleChangePassword}>
                                <div className="admin-form-group">
                                    <label className="admin-label">New Password</label>
                                    <input className="admin-input" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Enter new password" />
                                </div>
                                ...
                                <div className="admin-modal-actions">
                                    <button type="submit" className="admin-modal-submit" disabled={isChangingPassword}>
                                        {isChangingPassword ? '⏳ Updating...' : '✅ Update Password'}
                                    </button>
                                    <button type="button" className="admin-modal-cancel" onClick={() => { setShowChangePasswordModal(false); setPasswordChangeError(''); setPasswordChangeSuccess(''); setNewPassword(''); setConfirmPassword(''); }}>Cancel</button>
                                </div>
                            </form>
                        </div>
                    </div>
