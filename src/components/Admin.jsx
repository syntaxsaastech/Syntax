// src/pages/Admin.js - Complete updated file with Meeting Management & Notifications
...
function Admin() {
    ...
    const [seenMeetingIds, setSeenMeetingIds] = useState(() => {
        try {
            const stored = localStorage.getItem('admin_seen_meetings');
    ...
    const [seenRequestIds, setSeenRequestIds] = useState(() => {
        try {
            const stored = localStorage.getItem('admin_seen_requests');
            return stored ? JSON.parse(stored) : [];
        } catch {
            return [];
        }
            ...
            const unsubscribe = onSnapshot(meetingsQuery, (snapshot) => {
                try {
                    ...
                    snapshot.forEach((doc) => {
                        ...
                        // Create notification for new meeting requests
                        if (!seenMeetingIds.includes(data.id)) {
                    ...
                    if (newNotifications.length > 0) {
                        // Mark these meetings as seen
                        const newSeenIds = [...seenMeetingIds, ...newNotifications.map(n => n.meetingId)];
                        setSeenMeetingIds(newSeenIds);
                        try {
                            localStorage.setItem('admin_seen_meetings', JSON.stringify(newSeenIds));
                        } catch (e) {
    ...
    }, [isAuthenticated, seenMeetingIds]);
            ...
            const unsubscribe = onSnapshot(q, (snapshot) => {
                try {
                    ...
                    snapshot.forEach((doc) => {
                        ...
                        // Create notification for new client requests
                        if (!seenRequestIds.includes(data.id)) {
                            newNotifications.push({
                                id: `request-${data.id}`,
                                type: 'request',
                                requestId: data.id,
                    ...
                    if (newNotifications.length > 0) {
                        const newSeenIds = [...seenRequestIds, ...newNotifications.map(n => n.requestId)];
                        setSeenRequestIds(newSeenIds);
                        try {
                            localStorage.setItem('admin_seen_requests', JSON.stringify(newSeenIds));
                        } catch (e) {
                            console.error('localStorage error:', e);
                        }
    ...
    }, [isAuthenticated, seenRequestIds]);
