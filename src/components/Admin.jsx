// src/pages/Admin.js - COMPLETE FILE + Add History feature
...
function Admin() {
    ...
    // Effect options for history (same effects as the History page animations)
    ...
    // ============ LOAD SEEN IDS FROM LOCALSTORAGE (ONE TIME) ============
    useEffect(() => {
        try {
    ...
    // ============ FETCH MEETINGS + CREATE NOTIFICATIONS ============
    useEffect(() => {
        if (!isAuthenticated) return;
                    ...
                    if (changed) {
                        try {
                            localStorage.setItem(
                                'admin_seen_meetings',
                                JSON.stringify([...seenMeetingIdsRef.current])
                            );
                        } catch (e) {
    ...
    // ============ FETCH ALL CLIENT REQUESTS + NOTIFICATIONS ============
    useEffect(() => {
        if (!isAuthenticated) return;
        if (!hasLoadedSeenIdsRef.current) return;
                ...
                try {
                    ...
                    if (changed) {
                        try {
                            localStorage.setItem(
                                'admin_seen_requests',
                                JSON.stringify([...seenRequestIdsRef.current])
                            );
                        } catch (e) {
                            console.error('localStorage error:', e);
                        }
    ...
    // Upload history image to Cloudinary
    ...
    // Upload image to Cloudinary (developers - unchanged)
        ...
        <div style={{
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <div ref={notificationRef} style={{ position: 'relative' }}>
                        <button
                            ...
                            style={{
                                padding: '10px 16px',
                                background: unreadCount > 0 ? 'rgba(255, 107, 107, 0.15)' : 'rgba(255,255,255,0.05)',
                                border: unreadCount > 0 ? '1px solid rgba(255, 107, 107, 0.4)' : '1px solid rgba(255,255,255,0.1)',
                                borderRadius: '10px',
                                color: unreadCount > 0 ? '#FF6B6B' : '#A8B2D1',
                                cursor: 'pointer',
                                fontWeight: '500',
                                fontSize: '0.95rem',
                                display: 'flex',
                        ...
                        {showNotifications && (
                            <div style={{
                                ...
                                <div style={{
                                    ...
                                    <h3 style={{ color: '#FFD93D', margin: 0, fontSize: '1rem' }}>
                                        📬 Notifications
                                    </h3>
                                    <div style={{ display: 'flex', gap: '8px' }}>
                                        {notifications.length > 0 && (
                                            <>
                                                <button onClick={markAllAsRead} style={{ background: 'none', border: 'none', color: '#4D96FF', cursor: 'pointer', fontSize: '0.75rem', padding: '4px 8px' }}>
                                                    Mark all read
                                                </button>
                                                <button onClick={clearAllNotifications} style={{ background: 'none', border: 'none', color: '#FF6B6B', cursor: 'pointer', fontSize: '0.75rem', padding: '4px 8px' }}>
                                                    Clear all
                                                </button>
                                            </>
                                        )}
                                ...
                                {notifications.length === 0 ? (
                                    <div style={{ padding: '40px 20px', textAlign: 'center', color: '#666' }}>
                                        ...
                                        <p style={{ margin: '5px 0' }}>No notifications yet</p>
                                        <p style={{ margin: '5px 0', fontSize: '0.8rem' }}>
                                            New meeting requests and client requests will appear here
                                        </p>
                                    </div>
                                ) : (
                                    notifications.map((notification) => (
                                        <div
                                            ...
                                            style={{
                                                padding: '14px 20px',
                                                borderBottom: '1px solid rgba(255, 107, 107, 0.05)',
                                                cursor: 'pointer',
                                                display: 'flex',
                                                alignItems: 'flex-start',
                                                gap: '12px',
                                                background: notification.read ? 'transparent' : 'rgba(255, 217, 61, 0.03)',
                                                borderLeft: notification.read ? '3px solid transparent' : '3px solid #FFD93D',
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                <p style={{ color: '#FFD93D', fontSize: '0.85rem', fontWeight: '600', margin: '0 0 4px 0' }}>
                                                    {notification.title}
                                                </p>
                                                <p style={{ color: '#E0E0E0', fontSize: '0.85rem', margin: '0 0 6px 0', lineHeight: '1.4' }}>
                                                    {notification.message}
                                                </p>
                                                {notification.type === 'meeting' && notification.meetingDate && (
                                                    <p style={{ color: '#4D96FF', fontSize: '0.75rem', margin: '0 0 4px 0' }}>
                                                        📅 {notification.meetingDate} at {notification.meetingTime}
                                                    </p>
                                                )}
                                                {notification.clientEmail && (
                                                    <p style={{ color: '#A8B2D1', fontSize: '0.75rem', margin: '0 0 4px 0' }}>
                                                        📧 {notification.clientEmail}
                                                    </p>
                                                )}
            ...
            {showAnalytics && (
                <div style={{
                    ...
                    <h2 style={{ color: '#4D96FF', marginTop: 0 }}>📊 Analytics Overview</h2>

                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
                        <div>
                            <h3 style={{ color: '#FFD93D', fontSize: '1rem', marginBottom: '12px' }}>🎯 Service Distribution</h3>
                                    ...
                                    <div key={item.name} style={{ marginBottom: '8px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                                            <span style={{ color: '#A8B2D1', fontSize: '0.85rem' }}>
                                                {serviceTypes[item.name] || item.name}
                                            </span>
                                            <span style={{ color: '#FFD93D', fontSize: '0.85rem', fontWeight: 'bold' }}>{item.value}</span>
                                        </div>
                        ...
                        <div>
                            <h3 style={{ color: '#FFD93D', fontSize: '1rem', marginBottom: '12px' }}>⚡ Priority Distribution</h3>
                            {priorityData.map((item) => (
                                <div key={item.name} style={{ marginBottom: '8px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                                        <span style={{ color: '#A8B2D1', fontSize: '0.85rem' }}>{getPriorityBadge(item.name).label}</span>
                                        <span style={{ color: '#FFD93D', fontSize: '0.85rem', fontWeight: 'bold' }}>{item.value}</span>
                                    </div>
                                    <div style={{ background: 'rgba(255,255,255,0.05)', height: '6px', borderRadius: '3px', overflow: 'hidden' }}>
                                        <div style={{
                        ...
                        <div>
                            <h3 style={{ color: '#FFD93D', fontSize: '1rem', marginBottom: '12px' }}>📈 Status Distribution</h3>
                            {statusData.map((item) => (
                                <div key={item.name} style={{ marginBottom: '8px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
                                        <span style={{ color: '#A8B2D1', fontSize: '0.85rem' }}>{getStatusBadge(item.name).label}</span>
                                        <span style={{ color: '#FFD93D', fontSize: '0.85rem', fontWeight: 'bold' }}>{item.value}</span>
                                    </div>
                                    <div style={{ background: 'rgba(255,255,255,0.05)', height: '6px', borderRadius: '3px', overflow: 'hidden' }}>
                                        <div style={{
                    ...
                    {monthlyData.length > 0 && (
                        <div style={{ marginTop: '25px' }}>
                            <h3 style={{ color: '#FFD93D', fontSize: '1rem', marginBottom: '12px' }}>📅 Monthly Trend</h3>
                            <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '10px' }}>
                                {monthlyData.map((m) => (
                                    <div key={m.month} style={{
                                        ...
                                        <div style={{ color: '#4D96FF', fontSize: '0.8rem', marginBottom: '5px' }}>{m.month}</div>
                                        <div style={{ color: '#FFD93D', fontSize: '1.3rem', fontWeight: 'bold' }}>{m.total}</div>
                                        <div style={{ color: '#6BCB77', fontSize: '0.7rem' }}>✅ {m.completed}</div>
                                        <div style={{ color: '#FFD93D', fontSize: '0.7rem' }}>⏳ {m.pending}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <div style={{
                ...
                <div style={{ background: '#1A1E37', padding: '20px', borderRadius: '15px', border: '1px solid rgba(255, 107, 214, 0.15)' }}>
                    <p style={{ color: '#FF6BD6', margin: 0, fontSize: '0.9rem' }}>📜 History</p>
                    <h2 style={{ color: '#FF6BD6', margin: '5px 0 0 0' }}>{historyItems.length}</h2>
                </div>
            </div>
            ...
            <div style={{
                ...
                background: '#1A1E37',
                borderRadius: '15px',
                border: '1px solid rgba(255, 107, 214, 0.2)',
                padding: '25px'
            }}>
                <div style={{
                    display: 'flex',
                    ...
                    <div>
                        <h2 style={{ color: '#FF6BD6', margin: 0 }}>📜 History / Journey</h2>
                        <p style={{ color: '#A8B2D1', fontSize: '0.85rem', margin: '4px 0 0 0' }}>
                            {historyItems.length} item{historyItems.length !== 1 ? 's' : ''} • Shows on the History page
                        </p>
                    </div>
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                        <button
                            ...
                            style={{
                                padding: '8px 16px',
                                background: showHistorySection ? 'rgba(255, 107, 107, 0.15)' : 'rgba(255, 107, 214, 0.15)',
                                border: `1px solid ${showHistorySection ? 'rgba(255, 107, 107, 0.3)' : 'rgba(255, 107, 214, 0.3)'}`,
                                borderRadius: '8px',
                                color: showHistorySection ? '#FF6B6B' : '#FF6BD6',
                                cursor: 'pointer',
                                fontWeight: '500',
                                fontSize: '0.85rem'
                            }}
                ...
                {showHistorySection && (
                    historyItems.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '40px 0', color: '#666' }}>
                            <span style={{ fontSize: '3rem', display: 'block', marginBottom: '10px' }}>📜</span>
                            <p>No history items yet. Click "Add History Item" to create your first one.</p>
                            <p style={{ fontSize: '0.8rem', color: '#888', marginTop: '8px' }}>
                                These will appear on the public <strong>History</strong> page in the same order.
                            </p>
                        </div>
                    ) : (
                        <div style={{
                            ...
                            {historyItems.map((item) => (
                                <div
                                    key={item.id}
                                    style={{
                                        background: 'rgba(255, 107, 214, 0.03)',
                                        borderRadius: '12px',
                                ...
                                >
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                                        {item.imageUrl ? (
                                            <img
                                                ...
                                                style={{
                                                    ...
                                                    borderRadius: '50%',
                                                    objectFit: 'cover',
                                                    border: '2px solid rgba(255, 107, 214, 0.4)',
                                                    flexShrink: 0
                                                }}
                                            />
                                        ) : (
                                        ...
                                        <div style={{ minWidth: 0, flex: 1 }}>
                                            <h3 style={{ color: '#E0E0E0', margin: 0, fontSize: '1rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                {item.title}
                                            </h3>
                                            <p style={{ color: '#FF6BD6', margin: '2px 0 0 0', fontSize: '0.75rem' }}>
                                                Effect: {item.effect}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div style={{
                                        ...
                                        marginTop: '12px',
                                        paddingTop: '12px',
                                        borderTop: '1px solid rgba(255, 107, 214, 0.08)',
                                        flexWrap: 'wrap'
                                    }}>
                                        <button
            ...
            <div id="requests-section" style={{ marginBottom: '25px' }}>
            ...
            {showHistoryModal && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(10, 14, 39, 0.95)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
                    <div style={{ background: '#1A1E37', padding: '30px', borderRadius: '20px', maxWidth: '600px', width: '100%', maxHeight: '90vh', overflow: 'auto', border: '1px solid rgba(255, 107, 214, 0.3)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <h2 style={{ color: '#FF6BD6', margin: 0 }}>
                                {editingHistory ? '✏️ Edit History Item' : '📜 Add History Item'}
                            </h2>
                            <button
                                onClick={() => {
                                    setShowHistoryModal(false);
                        ...
                        <form onSubmit={saveHistory}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                <div style={{ marginBottom: '15px' }}>
                                    <label style={{ display: 'block', marginBottom: '5px', color: '#A8B2D1' }}>Year *</label>
                                    <input
                                        type="text"
                                        name="year"
                                        ...
                                        style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255, 107, 214, 0.2)', borderRadius: '10px', color: '#E0E0E0' }}
                                    />
                                </div>
                                <div style={{ marginBottom: '15px' }}>
                                    <label style={{ display: 'block', marginBottom: '5px', color: '#A8B2D1' }}>Badge (optional)</label>
                                    <input
                                        type="text"
                                        name="badge"
                                        value={historyFormData.badge}
                                        onChange={handleHistoryChange}
                                        placeholder="e.g., 🚀 Latest, 🌍 Global"
                                        style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255, 107, 214, 0.2)', borderRadius: '10px', color: '#E0E0E0' }}
                                    />
                                </div>
                            </div>
                            ...
                            <div style={{ marginBottom: '15px' }}>
                                <label style={{ display: 'block', marginBottom: '5px', color: '#A8B2D1' }}>Title *</label>
                                <input
                                    type="text"
                                    name="title"
                                    ...
                                    onChange={handleHistoryChange}
                                    required
                                    placeholder="e.g., Franchise Finder"
                                    style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255, 107, 214, 0.2)', borderRadius: '10px', color: '#E0E0E0' }}
                                />
                            </div>
                            ...
                            <div style={{ marginBottom: '15px' }}>
                                ...
                                <textarea
                                    ...
                                    onChange={handleHistoryChange}
                                    required
                                    placeholder="Describe the project/milestone..."
                                    style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255, 107, 214, 0.2)', borderRadius: '10px', color: '#E0E0E0', minHeight: '80px', fontFamily: 'inherit' }}
                                />
                            </div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                ...
                                <div style={{ marginBottom: '15px' }}>
                                    <label style={{ display: 'block', marginBottom: '5px', color: '#A8B2D1' }}>Animation Effect *</label>
                                    <select
                                        name="effect"
                                        value={historyFormData.effect}
                                        onChange={handleHistoryChange}
                                        style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255, 107, 214, 0.2)', borderRadius: '10px', color: '#E0E0E0', cursor: 'pointer' }}
                                    >
                            ...
                            <div style={{ marginBottom: '15px' }}>
                                <label style={{ display: 'block', marginBottom: '5px', color: '#A8B2D1' }}>Custom Image (optional)</label>
                                <input
                                    type="file"
                                    ref={historyFileInputRef}
                                    accept="image/*"
                                    onChange={handleHistoryImageUpload}
                                    disabled={historyUploadingImage}
                                    style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,107,214,0.05)', border: '1px solid rgba(255,107,214,0.2)', borderRadius: '10px', color: '#FF6BD6', cursor: 'pointer', fontSize: '0.9rem' }}
                                />
                                {historyUploadingImage && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#FF6BD6', marginTop: '8px' }}>
                                        <span>⏳ Uploading... {historyUploadProgress}%</span>
                                    </div>
                                )}
                                {historyImageUrl && (
                                    <div style={{ marginTop: '10px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                        <img src={historyImageUrl} alt="Preview" style={{ width: '60px', height: '60px', borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255, 107, 214, 0.3)' }} />
                                        <button
                                            type="button"
                                            onClick={() => setHistoryImageUrl('')}
                                            style={{ padding: '4px 12px', background: 'rgba(255, 107, 107, 0.15)', border: '1px solid rgba(255, 107, 107, 0.3)', borderRadius: '6px', color: '#FF6B6B', cursor: 'pointer', fontSize: '0.8rem' }}
                                        >
                                            Remove Image
                                        </button>
                                    </div>
                                ...
                                <p style={{ color: '#666', fontSize: '0.75rem', marginTop: '6px' }}>
                                    Leave empty to use the emoji icon instead.
                                </p>
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '15px', marginTop: '20px' }}>
                                ...
                                <button
                                    ...
                                    style={{ padding: '12px 24px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255, 107, 214, 0.2)', borderRadius: '10px', color: '#A8B2D1', cursor: 'pointer' }}
                                >
                                    Cancel
                                </button>
                            </div>
            ...
            {showMeetingDetail && selectedMeeting && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(10, 14, 39, 0.95)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000, padding: '20px' }}>
                    <div style={{ background: '#1A1E37', padding: '35px', borderRadius: '20px', maxWidth: '550px', width: '100%', maxHeight: '90vh', overflow: 'auto', border: '1px solid rgba(77, 150, 255, 0.3)' }}>
