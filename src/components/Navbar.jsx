...
function Navbar() {
    ...
    // Listen for notifications from clientRequests, meetings, and assignments
    useEffect(() => {
        if (!user) return;

        const unsubscribers = [];

        // Listen to clientRequests for status changes
        ...
        const unsubRequests = onSnapshot(requestsQuery, (snapshot) => {
            ...
            snapshot.forEach((doc) => {
                ...
                // Check for status changes (approved, rejected, in-progress, completed)
                if (data.status && ['approved', 'rejected', 'in-progress', 'completed'].includes(data.status)) {
                    const notificationId = `request-${data.id}-${data.status}`;
        ...
        // Listen to meetings for status changes
        ...
        const unsubMeetings = onSnapshot(meetingsQuery, (snapshot) => {
            ...
            snapshot.forEach((doc) => {
                ...
                if (data.status && ['confirmed', 'cancelled', 'completed'].includes(data.status)) {
                    const notificationId = `meeting-${data.id}-${data.status}`;
                    const statusMessages = {
        ...
        // Listen to clientRequests for developer assignments
        const assignmentQuery = query(
            collection(db, 'clientRequests'),
            where('userId', '==', user.uid)
        );

        const unsubAssignments = onSnapshot(assignmentQuery, (snapshot) => {
            const newNotifications = [];
            
            snapshot.forEach((doc) => {
                const data = { id: doc.id, ...doc.data() };
                
                // Check if developers were assigned
                if (data.assignedDevelopers && data.assignedDevelopers.length > 0) {
                    ...
                    const notificationId = `assign-${data.id}-${data.assignedDevelopers.length}`;
    ...
    const getNotificationColor = (type, status) => {
        if (type === 'assignment') return '#FFD93D';
        if (type === 'meeting') {
            const colors = { 'confirmed': '#6BCB77', 'cancelled': '#FF6B6B', 'completed': '#4D96FF' };
            return colors[status] || '#4D96FF';
        }
        const colors = { 'approved': '#6BCB77', 'rejected': '#FF6B6B', 'in-progress': '#4D96FF', 'completed': '#6BCB77' };
    ...
    const handleNotificationClick = (notification) => {
        ...
        if (notification.type === 'request' || notification.type === 'assignment') {
            navigate('/client');
